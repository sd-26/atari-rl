...
class Model(ModelDesc):
    ...
    def build_graph(self, comb_state, action, reward, isOver):
        ...
        pred_action_value = tf.reduce_sum(input_tensor=self.predict_value * action_onehot, axis=1)  # N,
        max_pred_reward = tf.reduce_mean(input_tensor=tf.reduce_max(
            input_tensor=self.predict_value, axis=1), name='predict_reward')
        summary.add_moving_summary(max_pred_reward)

        with tf.compat.v1.variable_scope('target'), varreplace.freeze_variables(skip_collection=True):
            targetQ_predict_value = self.get_DQN_prediction(next_state)    # NxA

        if self.method != 'Double':
            # DQN
            best_v = tf.reduce_max(input_tensor=targetQ_predict_value, axis=1)    # N,
        else:
            # Double-DQN
            next_predict_value = self.get_DQN_prediction(next_state)
            self.greedy_choice = tf.argmax(input=next_predict_value, axis=1)   # N,
            predict_onehot = tf.one_hot(self.greedy_choice, self.num_actions, 1.0, 0.0)
            best_v = tf.reduce_sum(input_tensor=targetQ_predict_value * predict_onehot, axis=1)
    ...
    def optimizer(self):
        lr = tf.compat.v1.get_variable('learning_rate', initializer=1e-3, trainable=False)
        tf.compat.v1.summary.scalar("learning_rate-summary", lr)
        opt = tf.compat.v1.train.RMSPropOptimizer(lr, decay=0.95, momentum=0.95, epsilon=1e-2)
